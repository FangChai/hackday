...
def gen(json_fn, out_file):
    f = open(json_fn, "r")
    map_list = json.load(f)
    print(map_list["operand1"])
    print(map_list["operand2"])
    try:
        gen_status_vars(map_list["operand1"])
        gen_status_vars(map_list["operand2"])
    except KeyError:
        pass
